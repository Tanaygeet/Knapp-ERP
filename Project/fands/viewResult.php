...
?>
        ...
        <head>
            ...
            <title><?php echo "$fname $lname"; ?> Result</title>
        </head>
                    ...
                    <nav class="filter">
                        <form action='<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?><?php echo "?fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role"; ?>' method="POST">
                            <select name="s1" id="s1" class="dropDown">
                                ...
                                <?php
                                include '../DB/db.php';
                                $query1 = "select distinct semester from result where erpid = '$erpid';";
                            ...
                            <select name="s3" id="s3" class="dropDown">
                                ...
                                <?php
                                $query3 = "select distinct course from result where erpid = '$erpid';";
                            ...
                            <select name="s4" id="s4" class="dropDown">
                                ...
                                <?php
                                $query4 = "select distinct assignment from result where erpid = '$erpid';";
                                $run4 = mysqli_query($connection, $query4);
                                if (mysqli_num_rows($run4) > 0) {
                                    while ($row = mysqli_fetch_assoc($run4)) {
                                        echo "<option value='$row[assignment]'>$row[assignment]</option>";
                                    }
                                } else {
                                    echo "<option value='all'>-</option>";
                                }
                    ...
                    <div class="allotForm">
                        <div class="tableContainer">
                            ...
                            <table class="table tableForm">
                                ...
                                <tbody>
                                    <?php
                                    $query4 = "select * from result where erpid = '$erpid';";
                                    $run4 = mysqli_query($connection, $query4);
                                    if (isset($_POST["filter"])) {
                                        $s1 = $_POST["s1"];
                                        $s3 = $_POST["s3"];
                                        $s4 = $_POST["s4"];
                                        if ($s1 == "all" && $s3 == "all" && $s4 == "all") {
                                            $query4 = "select * from result where erpid = '$erpid'";
                                            $run4 = mysqli_query($connection, $query4);
                                        } elseif ($s1 != "all" && $s3 == "all" && $s4 == "all") {
                                            $query4 = "SELECT * FROM result WHERE erpid = '$erpid' AND semester = '$s1';";
                                            $run4 = mysqli_query($connection, $query4);
                                        } elseif ($s1 == "all" && $s3 != "all" && $s4 == "all") {
                                            $query4 = "select * from result where erpid = '$erpid' and course='$s3';";
                                            $run4 = mysqli_query($connection, $query4);
                                        } elseif ($s1 == "all"  && $s3 == "all" && $s4 != "all") {
                                            $query4 = "select * from result where erpid = '$erpid' and assignment='$s4';";
                                            $run4 = mysqli_query($connection, $query4);
                                        } elseif ($s1 != "all" && $s3 != "all" && $s4 == "all") {
                                            $query4 = "select * from result where erpid = '$erpid' and semester='$s1' and course='$s3';";
                                            $run4 = mysqli_query($connection, $query4);
                                        } elseif ($s1 != "all" && $s3 == "all" && $s4 != "all") {
                                            $query4 = "select * from result where erpid = '$erpid' and semester='$s1' and assignment='$s4';";
                                            $run4 = mysqli_query($connection, $query4);
                                        } elseif ($s1 == "all" && $s3 != "all" && $s4 != "all") {
                                            $query4 = "select * from result where erpid = '$erpid' and course='$s3' and assignment='$s4';";
                                            $run4 = mysqli_query($connection, $query4);
                                        } else {
                                            $query4 = "select * from result where erpid = '$erpid' and semester='$s1' and course='$s3' and assignment='$s4';";
                                            $run4 = mysqli_query($connection, $query4);
                                        }
                                    }
                                    if (mysqli_num_rows($run4) > 0) {
