...
if ($_SESSION['sid'] && $_SESSION['skey']) {
    ...
    if ($_SESSION['sid'] == $erpid && $_SESSION['skey'] == $passwd) {
        ...
        $role = $_GET["role"];
        include "../DB/db.php";
        $fee = "SELECT payment FROM accounts where erpid = '$erpid' and paid != 'paid';";
        $feeGet = mysqli_query($connection, $fee);
        if (mysqli_num_rows($feeGet) > 0) {
            while ($row = mysqli_fetch_assoc($feeGet)) {
                $pay = $row['payment'];
            }
        } else {
            $pay = 0;
...
?>
        ...
        <section class="main">
            <div class="sidebar">
                <form action='../notice/smain.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role"; ?>' method="POST"><button type="submit">Notices</button></form>
                <form action="../events/student-event.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role" ?>" target="_blank" method="POST"><button type="submit">Events</button></form>
                <form action="../chat/smessage.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role" ?>" target="_blank" method="POST"><button type="submit">KNAPP Chats</button>
                </form>
                ...
                <form action="../fands/student-assignment.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role" ?>" target="_blank" method="POST"><button type="submit">Assignments</button></form>
                <form action="../fands/viewAtt.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role" ?>" target="_blank" method="POST"><button type="submit">Attendence</button></form>
                <form action="../fands/viewResult.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role" ?>" target="_blank" method="POST"><button type="submit">Result</button></form>
                <!-- <form action="#" method="POST"><button type="submit">Books</button></form> -->
                <form action="../fee/stfee.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role&pay=$pay" ?>" method="POST">
                    <?php
                    if ($pay == 0) {
                    ?>
                        <button type="submit" disabled>Fees Payment <small style="color: green; font-weight: bold;">(₹ <?php echo $pay; ?>)</small></button>
                    <?php
                    } else {
                    ?>
                        <button type="submit">Fees Payment <small style="color: red; font-weight: bold;">(₹ <?php echo $pay; ?>)</small></button>
                    <?php
                    }
            ...
            <section class="dashRight">
                <form action="../fands/viewAtt.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role" ?>" target="_blank" method="POST">
                    <button type="submit">
                ...
                <form action="../fands/student-assignment.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role" ?>" target="_blank" method="POST">
                    <button type="submit">
                ...
                <form action="../fee/stfee.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role&pay=$pay" ?>" method="POST">
                    ...
                    ?>
                        <button type="submit" disabled>
                            <div class="customCard" id="c5">
                                Fees Payment <br><small style="color: green; font-weight: bold;">To pay ₹ <?php echo $pay; ?></small>
                            </div>
                        </button>
                    <?php
                    } else {
                    ?>
                        <button type="submit">
                            <div class="customCard" id="c5">
                                Fees Payment <br><small style="color: red; font-weight: bold;">To pay ₹ <?php echo $pay; ?></small>
                            </div>
                        </button>
                    <?php
                    }
                ...
                <form action="../fands/viewResult.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role" ?>" target="_blank" method="POST">
                    <button type="submit">
