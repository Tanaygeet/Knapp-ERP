                ...
                <div class="eventBox">
                    ...
                    if (mysqli_num_rows($runEvents) > 0) {
                        while ($row = mysqli_fetch_assoc($runEvents)) {
                            ...
                            $today = date("Y-m-d");
                            if ($endDate >= $today) {
                                echo "<a href='showReg.php?eventName=$row[event_name]&fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role' target='_blank'>
...
?>
                ...
                <span class="btn">
                    <a href='host_event.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role"; ?>' class="add">Host event</a>
                </span>
            </div>
        </body>
