...
?>
        ...
        <body>
            ...
            <section class="main">
                <div class="sidebar">
                    <form action='../notice/main.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role"; ?>' method="POST"><button type="submit">Notices</button></form>
                    <form action="../events/main-event.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role" ?>" method="POST" target="_blank"><button type="submit">Events</button></form>
                ...
                <section class="showContainer">
                    <nav class="filter">
                        <form action='<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?><?php echo "?fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role"; ?>' method="POST">
                            <select name="s1" id="s1" class="dropDown">
                                ...
                                <?php
                                include '../DB/db.php';
                                $query1 = "select distinct year from accounts where role = 'admin' order by year;";
                            ...
                            <select name="s2" id="s2" class="dropDown">
                                ...
                                <?php
                                $query2 = "select distinct degree from accounts where role = 'admin' order by degree;";
                            ...
                            <select name="s3" id="s3" class="dropDown">
                                ...
                                <?php
                                $query3 = "select distinct branch from accounts where role = 'admin' order by branch;";
                    ...
                    <div class="tableContainer">
                        ...
                        <table class="table table-borderless">
                            ...
                            <tbody>
                                <?php
                                $query4 = "select * from accounts where role = 'admin' order by erpid;";
                                ...
                                if (isset($_POST["filter"])) {
                                    ...
                                    if ($s1 == "all" && $s2 == "all" && $s3 == "all") {
                                        ...
                                        $query4 = "select * from accounts where role = 'admin' and year='$s1' order by erpid;";
                                        $run4 = mysqli_query($connection, $query4);
                                    } elseif ($s1 == "all" && $s2 != "all" && $s3 == "all") {
                                        $query4 = "select * from accounts where role = 'admin' and degree='$s2' order by erpid;";
                                        $run4 = mysqli_query($connection, $query4);
                                    } elseif ($s1 == "all" && $s2 == "all" && $s3 != "all") {
                                        $query4 = "select * from accounts where role = 'admin' and branch='$s3' order by erpid;";
                                        $run4 = mysqli_query($connection, $query4);
                                    } elseif ($s1 != "all" && $s2 != "all" && $s3 == "all") {
                                        $query4 = "select * from accounts where role = 'admin' and year='$s1' and degree='$s2' order by erpid;";
                                        $run4 = mysqli_query($connection, $query4);
                                    } elseif ($s1 != "all" && $s2 == "all" && $s3 != "all") {
                                        $query4 = "select * from accounts where role = 'admin' and year='$s1' and branch='$s3' order by erpid;";
                                        $run4 = mysqli_query($connection, $query4);
                                    } elseif ($s1 == "all" && $s2 != "all" && $s3 != "all") {
                                        $query4 = "select * from accounts where role = 'admin' and degree='$s2' and branch='$s3' order by erpid;";
                                        $run4 = mysqli_query($connection, $query4);
                                    } else {
                                        $query4 = "select * from accounts where role = 'admin' and year='$s1' and degree='$s2' and branch='$s3' order by erpid;";
                                        $run4 = mysqli_query($connection, $query4);
                                    }
                                }
                                if (mysqli_num_rows($run4) > 0) {
                                    ...
                                    while ($row = mysqli_fetch_assoc($run4)) {
                                        ...
                                        <td>$row[degree]</td>
                                        <td>$row[branch]</td>
                                        <td><a href ='../DB/edit.php?fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role&uerpid=$row[erpid]&ufn=$row[firstname]&uln=$row[lastname]&uyear=$row[year]&info1=$row[degree]&info2=$row[branch]&uemail=$row[mailid]&umobile=$row[mobile]&location=showAdmins'><i class='fas fa-edit'></i></a></td>
                                        <td><a href ='../DB/delete.php?fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role&uerpid=$row[erpid]&location=showadmins'><i class='fas fa-trash-alt'></i></a></td>
                                    </tr>";
                                        $no++;
                                    }
                                } else {
