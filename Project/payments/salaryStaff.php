        ...
        <body>
            ...
            <section class="main">
                ...
                <section class="showContainer">
                    <nav class="filter">
                        <form action="#" method="POST">
                            <select name="s1" id="s1" class="dropDown">
                                ...
                                <?php
                                include "../DB/db.php";
                                $query1 = "select distinct year from accounts where role = 'staffmember' order by year;";
                            ...
                            <select name="s2" id="s2" class="dropDown">
                                ...
                                <?php
                                $query1 = "select distinct staff_type from accounts where role = 'staffmember' order by staff_type;";
                                $run1 = mysqli_query($connection, $query1);
                                if (mysqli_num_rows($run1) > 0) {
                                    while ($row = mysqli_fetch_assoc($run1)) {
                                        echo "<option value='$row[staff_type]'>$row[staff_type]</option>";
                                    }
                                } else {
                                    echo "<option value='all'>-</option>";
                                }
                            ...
                            <select name="s3" id="s3" class="dropDown">
                                ...
                                <?php
                                $query1 = "select distinct staff_shift from accounts where role = 'staffmember' order by staff_shift;";
                                $run1 = mysqli_query($connection, $query1);
                                if (mysqli_num_rows($run1) > 0) {
                                    while ($row = mysqli_fetch_assoc($run1)) {
                                        echo "<option value='$row[staff_shift]'>$row[staff_shift]</option>";
                                    }
                                } else {
                                    echo "<option value='all'>-</option>";
                                }
                    ...
                    <form action="../DB/postSal.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role"; ?>&loc=salaryStaff" method="post">
                        <div class="tableContainer">
                            ...
                            <table class="table tableForm">
                                ...
                                <tbody>
                                    <?php
                                    $query4 = "select * from accounts where role = 'staffmember' order by erpid;";
                                    ...
                                    if (isset($_POST["filter"])) {
                                        ...
                                        if ($s1 == "all" && $s2 == "all" && $s3 == "all") {
                                            ...
                                            $query4 = "select * from accounts where role = 'staffmember' and year='$s1' order by erpid;";
                                            $run4 = mysqli_query($connection, $query4);
                                        } elseif ($s1 == "all" && $s2 != "all" && $s3 == "all") {
                                            $query4 = "select * from accounts where role = 'staffmember' and staff_type='$s2' order by erpid;";
                                            $run4 = mysqli_query($connection, $query4);
                                        } elseif ($s1 == "all" && $s2 == "all" && $s3 != "all") {
                                            $query4 = "select * from accounts where role = 'staffmember' and staff_shift='$s3' order by erpid;";
                                            $run4 = mysqli_query($connection, $query4);
                                        } elseif ($s1 != "all" && $s2 != "all" && $s3 == "all") {
                                            $query4 = "select * from accounts where role = 'staffmember' and year='$s1' and staff_type='$s2' order by erpid;";
                                            $run4 = mysqli_query($connection, $query4);
                                        } elseif ($s1 != "all" && $s2 == "all" && $s3 != "all") {
                                            $query4 = "select * from accounts where role = 'staffmember' and year='$s1' and staff_shift='$s3' order by erpid;";
                                            $run4 = mysqli_query($connection, $query4);
                                        } elseif ($s1 == "all" && $s2 != "all" && $s3 != "all") {
                                            $query4 = "select * from accounts where role = 'staffmember' and staff_type='$s2' and staff_shift='$s3' order by erpid;";
                                            $run4 = mysqli_query($connection, $query4);
                                        } else {
                                            $query4 = "select * from accounts where role = 'staffmember' and year='$s1' and staff_type='$s2' and staff_shift='$s3' order by erpid;";
                                            $run4 = mysqli_query($connection, $query4);
                                        }
                                    }
                                    if (mysqli_num_rows($run4) > 0) {
                                        ...
                                        while ($row = mysqli_fetch_assoc($run4)) {
                                            ...
                                            <td>$row[erpid]</td>
                                        <input type='text' name='fid$cno' value='$row[erpid]' hidden>
                                        <td>$row[firstname] $row[lastname]</td>
                                        <td>$row[year]</td>
                                        <td>$row[staff_type]</td>
                                        <td>$row[staff_shift]</td>
                                        <td>₹ <input name='sal$cno' value='$row[payment]' style='text-align: center; border: none; border-bottom: solid 1px; background: transparent; max-width: 9vh; padding: 5%;' autofocus placeholder='80000'></td>";
                                            if ($row['paid'] == null) {
                                                echo "<td style='color: red;'>Not Paid</td></tr>";
                                            } else {
                                    ...
                                    ?>
                                    <tr>
                                        <td colspan="8" style="background: transparent; border: none; cursor: default;"><button class="salBtn" type="submit">Post Salary</button></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
