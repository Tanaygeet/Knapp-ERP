...
if ($_SESSION['sid'] && $_SESSION['skey']) {
    ...
    if ($_SESSION['sid'] == $erpid && $_SESSION['skey'] == $passwd) {
        ...
        $sender = $_GET["sender"];
        include "../DB/db.php";
        $query = "SELECT * FROM knapp_mail where send_by='$sender' and receiver_mail = '$mail' and serial='$mailserial';";
        $run = mysqli_query($connection, $query);
        if (mysqli_num_rows($run)) {
            $idQuery = "SELECT * FROM knapp_mail_acc where mail='$sender';";
            $idRun = mysqli_query($connection, $idQuery);
            if (mysqli_num_rows($idRun)) {
                while ($idRow = mysqli_fetch_assoc($idRun)) {
                    $senderName = $idRow['name'];
                }
            } else {
?>
                <script>
