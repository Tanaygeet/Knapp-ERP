...
?>
        ...
        <head>
            ...
            <title>KNAPP Inbox - <?php echo "$mail"; ?></title>
        </head>
                    ...
                    <div class="mails">
                        <ul class="mail">
                            ...
                            if (isset($_POST['mail_search'])) {
                                $words = $_POST['word'];
                                $fetch_mail = "SELECT * FROM knapp_mail where receiver_mail = '$mail' and send_by !='$mail' and sender like '%$words%' or receiver_mail = '$mail' and send_by !='$mail' and subject like '%$words%' or receiver_mail = '$mail' and send_by !='$mail' and mail like '%$words%' or receiver_mail = '$mail' and send_by !='$mail' and sender_mail like '%$words%' order by serial desc;";
                                $run_fetch = mysqli_query($connection, $fetch_mail);
                                if (mysqli_num_rows($run_fetch) == 0) {
                                    $throw = 1;
                                }
                            } else {
                                $fetch_mail = "SELECT * FROM knapp_mail where receiver_mail = '$mail' and send_by !='$mail' order by serial desc;";
                                $run_fetch = mysqli_query($connection, $fetch_mail);
                            }
                            if (mysqli_num_rows($run_fetch) > 0) {
                                while ($row = mysqli_fetch_assoc($run_fetch)) {
                                    echo "<li class='label'><a href='read.php?mailno=$row[serial]&sender=$row[send_by]&fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role'>
                                        <ul>
                                            <li>$row[send_by]</li>
                                ...
                                echo "<li class='label'><a href='default_mail.php?mail=default&fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role'>
    <ul>
        <li>Knapp</li>
        <li class='sub'>Your account has been created successfully. You don't have any mails in your inbox. You can start conversations by clicking on compose.</li>
    </ul>
</a></li>";
                            }
                    ...
                    <footer class="footer">
                        <a href='sent.php?<?php echo "fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role"; ?>' class="bottombtn">Outbox</a>
                    ...
                    <div class="composepop">
                        ...
                        <form action="../maildb/insertmail.php?<?php echo "prev=mail.php&fn=$fname&ln=$lname&key=$passwd&id=$erpid&role=$role"; ?>" autocomplete="on" method="POST">
